...
func TestLoad(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			env: map[string]string{
				"YANDEX_CLI_TOKEN": "t1.xxx",
			},
			wantErr: "set YANDEX_CLI_CLOUD_ORG_ID for Yandex Cloud Organization, or YANDEX_CLI_ORG_ID for Yandex 360",
		},
		{
			name: "both org vars set",
			env: map[string]string{
				"YANDEX_CLI_TOKEN":        "t1.xxx",
				"YANDEX_CLI_CLOUD_ORG_ID": "cloud123",
				"YANDEX_CLI_ORG_ID":       "y360_456",
			},
			wantErr: "set exactly one of YANDEX_CLI_CLOUD_ORG_ID (Cloud) or YANDEX_CLI_ORG_ID (360), not both",
		},
		{
			...
			wantErr: "YANDEX_CLI_TOKEN not set; run: export",
		},
		{
		...
		{
			name: "cloud tenancy detected from YANDEX_CLI_CLOUD_ORG_ID",
			env: map[string]string{
				"YANDEX_CLI_TOKEN":        "t1.xxx",
				"YANDEX_CLI_CLOUD_ORG_ID": "cloud123",
			},
		...
		{
			name: "360 tenancy detected from YANDEX_CLI_ORG_ID",
			env: map[string]string{
				"YANDEX_CLI_TOKEN":  "y0_xxx",
				"YANDEX_CLI_ORG_ID": "y360_456",
			},
			wantCheck: func(t *testing.T, c Config) {
				if c.Tenancy != Y360 {
		...
		},
		{
			...
			env: map[string]string{
				"YANDEX_CLI_TOKEN":            "t1.xxx",
				"YANDEX_CLI_CLOUD_ORG_ID":     "cloud123",
				"YANDEX_CLI_TRACKER_BASE_URL": "https://t.example",
				"YANDEX_CLI_WIKI_BASE_URL":    "https://w.example",
			},
			wantCheck: func(t *testing.T, c Config) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for _, k := range []string{
				"YANDEX_CLI_TOKEN", "YANDEX_CLI_ORG_ID", "YANDEX_CLI_CLOUD_ORG_ID",
				"YANDEX_CLI_TRACKER_BASE_URL", "YANDEX_CLI_WIKI_BASE_URL",
				"YANDEX_CLI_USE_YC",
			} {
				t.Setenv(k, "")
			}
...
func TestLoad_YCFallback_OptInDisabled_ReturnsExistingError(t *testing.T) {
	clearAuthEnv(t)
	t.Setenv("YANDEX_CLI_CLOUD_ORG_ID", "cloud123")
	fake := &fakeYCExecutor{out: []byte("t1.should-not-be-used")}
	swapYCExec(t, fake)

	_, err := Load()
	if err == nil {
		t.Fatal("expected error when YANDEX_CLI_TOKEN unset and YANDEX_CLI_USE_YC not set")
	}
	if !strings.Contains(err.Error(), "YANDEX_CLI_TOKEN not set; run: export") {
		t.Errorf("error = %q, want existing manual-export hint", err.Error())
	}
	if !strings.Contains(err.Error(), "YANDEX_CLI_USE_YC=1") {
		t.Errorf("error = %q, want hint about YANDEX_CLI_USE_YC opt-in", err.Error())
	}
	if fake.called {
		t.Error("yc executor must not be called when YANDEX_CLI_USE_YC is unset")
	}
}
...
func TestLoad_YCFallback_360_NotInvoked(t *testing.T) {
	clearAuthEnv(t)
	t.Setenv("YANDEX_CLI_ORG_ID", "y360_456")
	t.Setenv("YANDEX_CLI_USE_YC", "1") // must be ignored on 360
	fake := &fakeYCExecutor{out: []byte("t1.should-not-be-used")}
	swapYCExec(t, fake)
...
func TestLoad_YCFallback_UserTokenWins(t *testing.T) {
	clearAuthEnv(t)
	t.Setenv("YANDEX_CLI_TOKEN", "t1.user-supplied")
	t.Setenv("YANDEX_CLI_CLOUD_ORG_ID", "cloud123")
	t.Setenv("YANDEX_CLI_USE_YC", "1")
	...
	if fake.called {
		t.Error("yc executor must not be called when YANDEX_CLI_TOKEN is already set")
	}
}
